...
half4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
    ...
    //OVERRIDE THE STANDARD SHADER FOR DIRECTIONAL LIGHT
    //----------------------------------------------------------
    c.rgb =  float3(0.5, 0.2, 0.2) * saturate(dot(s.normalWorld, gi.light.dir));
    c.rgb += float3(0.2, 0.5, 0.1) * max(1.0 - (2.5 * dot(s.normalWorld, -s.eyeVec)), 0); 
    c.rgb =  lerp(c.rgb, 0, fmod(i.pos.x * 0.25, 2) > 1);
    //----------------------------------------------------------
